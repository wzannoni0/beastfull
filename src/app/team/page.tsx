    ...
    <AppShell title="Team" subtitle="Build your referral structure">
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          ...
          <p className="rounded-lg bg-black/20 px-3 py-2 text-sm">{demoUser.referralCode}</p>
          <p className="mt-3 text-xs text-slate-400">Link referral</p>
          <p className="rounded-lg bg-black/20 px-3 py-2 text-xs text-slate-300">{demoUser.referralLink}</p>
        </Card>
      </div>
      ...
      <Card>
        ...
        <div className="grid gap-2">
          {teamMembers.map((member, i) => (
            <div key={member.username} className="flex items-center justify-between rounded-lg bg-black/20 px-3 py-2 text-sm">
              <span>
                #{i + 1} @{member.username}
              </span>
              <span className="text-blue-300">{member.level}</span>
            </div>
          ))}
        </div>
      </Card>
